...
def deduplicate_walks_by_anonymous_code(walks: torch.Tensor) -> torch.Tensor:
    if walks.numel() == 0:
        return walks
    keep = []
    seen_codes = set()
...
def run_experiment(
    ...
    hrw_trans = cut_crossing_transition_rate(hrw.all_walks, S_cut)
    rw_trans = cut_crossing_transition_rate(rw.walks, S_cut)

    hrw_starts = (
        torch.cat(hrw.starts_by_level, dim=0)
        if hrw.starts_by_level
        else torch.empty((0,), dtype=torch.long, device=device)
    )
    hrw_by_src = grouped_walks_by_source(hrw.all_walks, hrw_starts) if hrw_starts.numel() > 0 else {}
    rw_by_src = grouped_walks_by_source(rw.walks, rw.starts) if rw.starts.numel() > 0 else {}
    ...
    results = {
        ...
        "hrw_cut_transition_rate": hrw_trans,
        "rw_cut_transition_rate": rw_trans,
...
def visualize_results(G, S_cut, results, title_prefix=""):
    ...
    ax2.bar(["RW", "HRW"], [results["rw_cut_transition_rate"], results["hrw_cut_transition_rate"]])
    ax2.set_ylim(0, 0.2)
    ax2.set_title("Cut-Transition Rate")
    ax2.set_ylabel("Fraction of transitions crossing cut")

    ax3 = plt.subplot(2, 2, 3)
    ax3.bar(["RW", "HRW"], [results["rw_mean_eat"], results["hrw_mean_eat"]])
    ...
    out_path = Path(f"plots/{title_prefix}_rw_vs_hrw.png")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="barbell",
        choices=["barbell", "sbm", "lollipop", "grid", "regular", "er", "caveman", "ws"],
    )
    ...
    if args.graph == "barbell":
        ...
        G, S_cut = build_caveman_graph(num_cliques=5, clique_size=20)
    elif args.graph == "ws":
    ...
    visualize_results(G, S_cut, results, title_prefix=args.graph)
