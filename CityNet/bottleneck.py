...
def hierarchical_random_walk_fast(
    ...
    Each level:
        1. Repeat each frontier node M times.
        2. Launch M short random walks of length L from each frontier node.
        3. Use terminal nodes as the next-level frontier.

    This version tracks source lineage:
        each sampled walk knows which original seed it descends from.
    """
    ...
    for level in range(R + 1):
        ...
        if merge_terminal:
            # Note:
            # merge_terminal changes the stochastic distribution because repeated
            # terminal nodes are collapsed.
            #
            # For each unique terminal, we keep one associated source.
            # For clean lineage analysis, merge_terminal=False is recommended.
            unique_frontier, inverse = torch.unique(
                terminals,
                sorted=False,
                return_inverse=True,
            )
...
def standard_random_walk_budget_matched(
    ...
    """
    Budget-matched standard random walk.

    Samples as many walks of length walk_length as possible under the
    transition budget. If there is a remainder, one shorter walk is sampled
    and padded by repeating its terminal node.
    """
...
# ============================================================
# Cut and bottleneck construction
# ============================================================

def get_boundary_nodes_from_cut(G: nx.Graph, S_cut: set[int]) -> set[int]:
    """
    Returns nodes incident to at least one edge crossing S_cut.
    """
...
def build_barbell_graph(left_size=20, bridge_len=2):
    ...
    bridge_nodes = set(range(left_size, left_size + bridge_len))

    bottleneck_nodes = set(bridge_nodes)

    # Clique node connected to the bridge.
    bottleneck_nodes.add(left_size - 1)

    # First right-clique node connected to the bridge.
    bottleneck_nodes.add(left_size + bridge_len)

    return G, S_cut, bottleneck_nodes
...
def build_sbm_graph(sizes=(50, 50), p_in=0.12, p_out=0.01, seed=0):
    ...
    bottleneck_nodes = get_boundary_nodes_from_cut(G, S_cut)
...
def cut_crossing_probability(walks: torch.Tensor, S_cut: set[int]) -> float:
    ...
    This metric is walk-length sensitive.
    """
...
def cut_crossing_transition_rate(walks: torch.Tensor, S_cut: set[int]) -> float:
    """
    Number of cut-crossing transitions divided by total transitions.
    """
...
def source_cut_reachability(
    ...
    """
    Fraction of original seed nodes whose descendant walks reach the opposite side.
    """
...
def source_average_opposite_side_coverage(
    ...
    """
    For each source seed, counts unique opposite-side nodes reached.
    Returns average over sources.
    """
...
def bottleneck_hit_probability(
    walks: torch.Tensor,
    bottleneck_nodes: set[int],
) -> float:
    """
    Fraction of walk segments that visit at least one bottleneck node.
    """

    if walks.numel() == 0:
        return 0.0

    B = set(bottleneck_nodes)
    cnt = 0

    for w in walks.tolist():
        hit = any(int(x) in B for x in w)
        cnt += int(hit)

    return cnt / walks.size(0)


def bottleneck_hit_count(
    walks: torch.Tensor,
    bottleneck_nodes: set[int],
) -> int:
    """
    Number of walk segments that visit at least one bottleneck node.
    """
...
def bottleneck_visit_rate(
    walks: torch.Tensor,
    bottleneck_nodes: set[int],
) -> float:
    """
    Fraction of all visited positions that are bottleneck nodes.
    """
...
def source_bottleneck_reachability(
    walks: torch.Tensor,
    sources: torch.Tensor,
    original_seeds: torch.Tensor,
    bottleneck_nodes: set[int],
    ...
    """
    Fraction of original seeds whose descendant walks visit any bottleneck node.
    """
...
def source_average_bottleneck_hits(
    walks: torch.Tensor,
    sources: torch.Tensor,
    original_seeds: torch.Tensor,
    bottleneck_nodes: set[int],
    ...
    """
    For each source seed, counts total bottleneck visits by its descendant walks.
    Returns average over source seeds.
    """
    ...
    B = set(bottleneck_nodes)
    source_to_hits = {int(s): 0 for s in original_seeds.tolist()}

    for src, w in zip(sources.tolist(), walks):
        src = int(src)

        if src not in source_to_hits:
            continue

        for x in w.tolist():
            if int(x) in B:
                source_to_hits[src] += 1

    if not source_to_hits:
        return 0.0

    return float(np.mean(list(source_to_hits.values())))
...
def summarize_walk_method(
    ...
    summary = {
        ...
        # Bottleneck metrics
        f"{name}_bottleneck_hit_prob_segment": bottleneck_hit_probability(
            walks=walks,
            bottleneck_nodes=bottleneck_nodes,
        ),
        f"{name}_bottleneck_hit_count_segment": bottleneck_hit_count(
            walks=walks,
            bottleneck_nodes=bottleneck_nodes,
        ),
        f"{name}_bottleneck_visit_rate": bottleneck_visit_rate(
            walks=walks,
            bottleneck_nodes=bottleneck_nodes,
        ),
        f"{name}_source_bottleneck_reachability": source_bottleneck_reachability(
            walks=walks,
            sources=sources,
            original_seeds=original_seeds,
            bottleneck_nodes=bottleneck_nodes,
        ),
        f"{name}_source_avg_bottleneck_hits": source_average_bottleneck_hits(
            walks=walks,
            sources=sources,
            original_seeds=original_seeds,
            bottleneck_nodes=bottleneck_nodes,
...
def run_experiment(
    ...
    results = {
        ...
        "num_bottleneck_nodes": len(bottleneck_nodes),
        "bottleneck_nodes": sorted(list(bottleneck_nodes)),
...
def visualize_results(
    ...
    coverage_values = [results[f"{m}_coverage"] for m in methods]
    cut_segment_values = [results[f"{m}_cut_cross_prob_segment"] for m in methods]
    cut_transition_values = [results[f"{m}_cut_cross_rate_transition"] for m in methods]
    source_cut_values = [results[f"{m}_source_cut_reachability"] for m in methods]
    bottleneck_hit_values = [results[f"{m}_bottleneck_hit_prob_segment"] for m in methods]
    source_bottleneck_values = [results[f"{m}_source_bottleneck_reachability"] for m in methods]
    ...
    ax1.set_ylabel("Visited nodes / all nodes")

    ax2 = plt.subplot(2, 4, 2)
    ax2.bar(labels, cut_segment_values)
    ax2.set_ylim(0, 1.0)
    ax2.set_title("Segment Cut-Crossing Probability")
    ax2.set_ylabel("Crossing segments / segments")

    ax3 = plt.subplot(2, 4, 3)
    ax3.bar(labels, cut_transition_values)
    ax3.set_ylim(0, 1.0)
    ax3.set_title("Transition Cut-Crossing Rate")
    ax3.set_ylabel("Crossing transitions / transitions")

    ax4 = plt.subplot(2, 4, 4)
    ax4.bar(labels, source_cut_values)
    ax4.set_ylim(0, 1.0)
    ax4.set_title("Source Cut Reachability")
    ax4.set_ylabel("Seeds reaching opposite side")

    ax5 = plt.subplot(2, 4, 5)
    ax5.bar(labels, bottleneck_hit_values)
    ax5.set_ylim(0, 1.0)
    ax5.set_title("Segment Bottleneck Hit Probability")
    ax5.set_ylabel("Segments hitting bottleneck")

    ax6 = plt.subplot(2, 4, 6)
    ax6.bar(labels, source_bottleneck_values)
    ax6.set_ylim(0, 1.0)
    ax6.set_title("Source Bottleneck Reachability")
    ax6.set_ylabel("Seeds hitting bottleneck")

    ax7 = plt.subplot(2, 4, 7)
    eat_values = [results[f"{m}_source_level_mean_eat"] for m in methods]
    ax7.bar(labels, eat_values)
    ax7.set_title("Source-Level Mean EAT")
    ax7.set_ylabel("Steps inside sampled segments")
    ...
    ax8.set_title("Visited Nodes and Bottlenecks")

    plt.suptitle(f"{title_prefix} Budget-Matched RW vs HRW", fontsize=15)
    plt.tight_layout()

    out_path = Path(f"plots/{title_prefix}_rw_vs_hrw_bottleneck_metrics.png")
...
def print_results(results):
    print("\n=== Budget-Matched HRW vs RW with Bottleneck Metrics ===\n")
    ...
    metric_suffixes = [
        ...
        "bottleneck_hit_prob_segment",
        "bottleneck_hit_count_segment",
        "bottleneck_visit_rate",
        "source_bottleneck_reachability",
        "source_avg_bottleneck_hits",
    ...
    print("hrw uses multi-level length-L walks with source lineage.")
    print("segment cut probability is walk-length sensitive.")
    print("transition cut rate is more budget-faithful.")
    print("bottleneck hit probability measures whether walks discover transition nodes.")
    print("source bottleneck reachability is often the most meaningful metric for HRW.")
...
def main():
    ...
    if args.graph == "barbell":
        G, S_cut, bottleneck_nodes = build_barbell_graph(
            left_size=20,
            bridge_len=2,
        )

    elif args.graph == "sbm":
        G, S_cut, bottleneck_nodes = build_sbm_graph(
            sizes=(50, 50),
            p_in=0.12,
            p_out=0.01,
            seed=args.seed,
        ...
        G, S_cut, bottleneck_nodes = build_lollipop_graph(
            clique_size=20,
            path_len=30,
        )

    elif args.graph == "grid":
        G, S_cut, bottleneck_nodes = build_grid_graph(
            m=10,
            n=10,
        )

    elif args.graph == "regular":
        G, S_cut, bottleneck_nodes = build_random_regular_graph(
            num_nodes=100,
            degree=4,
            seed=args.seed,
        )
        ...
        G, S_cut, bottleneck_nodes = build_er_graph(
            num_nodes=100,
            p=0.05,
            seed=args.seed,
        )
        ...
        G, S_cut, bottleneck_nodes = build_caveman_graph(
            num_cliques=5,
            clique_size=20,
        )

    elif args.graph == "ws":
        G, S_cut, bottleneck_nodes = build_watts_strogatz_graph(
            num_nodes=100,
            k=6,
            p=0.05,
            seed=args.seed,
