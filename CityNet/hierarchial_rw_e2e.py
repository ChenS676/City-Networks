"""
hierarchial_rw_e2e.py
------------
Full end-to-end standalone training script for HierarchialRW (Heterogeneous Anonymized
Random-walk Transformer) for link prediction on graphs.

No external hierarchial_rw_model.py required — everything is self-contained.
...
Usage
-----
  python hierarchial_rw_e2e.py --data_name Cora --data_root data/Cora
  python hierarchial_rw_e2e.py --data_name CiteSeer --data_root data/CiteSeer
  python hierarchial_rw_e2e.py --data_name PubMed --data_root data/PubMed
"""
...
def get_config():
    parser = argparse.ArgumentParser(description="HierarchialRW End-to-End Link Prediction")
...
def evaluate_hits(evaluator_hit, pos_pred, neg_pred, k_list):
    """
    Compute Hits@K using OGB's ogbl-collab Evaluator.
    For each positive edge, counts how many of the neg_pred scores it outranks.

    Args:
        evaluator_hit : ogb.linkproppred.Evaluator(name='ogbl-collab')
        pos_pred      : (P,)   scores for positive edges
        neg_pred      : (N,)   scores for negative edges (shared pool)
        k_list        : list of K values, e.g. [1, 10, 50, 100]
    Returns:
        dict  {'Hits@1': float, 'Hits@10': float, ...}
    """
    result = {}
    for K in k_list:
        evaluator_hit.K = K
        # OGB expects y_pred_neg shape (P, N) — broadcast neg_pred for each positive
        hits = evaluator_hit.eval({
            'y_pred_pos': pos_pred,
            'y_pred_neg': neg_pred.unsqueeze(0).expand(pos_pred.size(0), -1),
        })
        result[f'Hits@{K}'] = hits['hits@{}'.format(K)]
    return result


def evaluate_mrr(evaluator_mrr, pos_pred, neg_pred_mat):
    """
    Compute MRR using OGB's ogbl-citation2 Evaluator.

    Args:
        evaluator_mrr : ogb.linkproppred.Evaluator(name='ogbl-citation2')
        pos_pred      : (P,)     scores for positive edges
        neg_pred_mat  : (P, N)   scores for N negatives per positive
    Returns:
        dict  {'MRR': float}
    """
    result = evaluator_mrr.eval({
        'y_pred_pos': pos_pred,
        'y_pred_neg': neg_pred_mat,
    })
    return {'MRR': result['mrr_list'].mean().item()}


def evaluate_auc(pred, true):
    """
    Compute ROC-AUC and Average Precision using sklearn.

    Args:
        pred : (P+N,) float tensor of scores
        true : (P+N,) int tensor of labels (1=positive, 0=negative)
    Returns:
        dict  {'AUC': float, 'AP': float}
    """
...
def get_metric_score(evaluator_hit, evaluator_mrr, pos_pred, neg_pred, k_list):
    ...
    Args:
        ...
        evaluator_mrr : ogb.linkproppred.Evaluator(name='ogbl-citation2')
        pos_pred      : (P,)  scores for positive edges
        neg_pred      : (N,)  scores for negative edges
        k_list        : list of K values for Hits@K
    Returns:
        dict with keys: Hits@K (for each K), MRR, AUC, AP
    """
    result = {}
    ...
    # --- MRR ---
    # ogbl-citation2 expects one neg row per positive: shape (P, N)
    neg_mat       = neg_pred.unsqueeze(0).expand(pos_pred.size(0), -1)
    result['MRR'] = evaluate_mrr(evaluator_mrr, pos_pred, neg_mat)['MRR']

    # --- AUC & AP ---
    pred = torch.cat([pos_pred, neg_pred])
    true = torch.cat([
        torch.ones(pos_pred.size(0),  dtype=torch.int),
        torch.zeros(neg_pred.size(0), dtype=torch.int),
    ])
    auc_scores    = evaluate_auc(pred, true)
    result['AUC'] = auc_scores['AUC']
    result['AP']  = auc_scores['AP']
...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        group=f"{config['data_name']} HierarchialRW {'MLP' if config['use_mlp'] else 'DotProduct'}",
        name=run_name,
        config=config,
    )
    run_id = wandb.run.id
    ...
    for epoch in range(config['num_epochs']):
        ...
        # Subsample train edges for evaluation (HierarchialRW style)
